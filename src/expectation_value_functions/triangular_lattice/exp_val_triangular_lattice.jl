function apply_loc_unitaries(hopp_term_array, unitary_paras; space_type = ℂ)
    ...
    id_mat = [1 0 ; 0 1]
    id = TensorMap(id_mat, space_type^2, space_type^2)

    iσ_y_mat = 0.5* [0 1 ; -1 0]
    iσ_y = TensorMap(iσ_y_mat, space_type^2, space_type^2)

    #display(iσ_y)
    hor_loc_unitary = cos((kx)/2) * id + sin((kx)/2) * 2 * iσ_y
    vert_loc_unitary = cos((ky)/2) * id + sin((ky)/2) * 2 * iσ_y
    diag_loc_unitary = cos((kx+ky)/2) * id + sin((kx+ky)/2) * 2 * iσ_y
...
function exp_val_energy_generic_triangular_lattice_model(env_arr, loc, loc_d, Ham_parameters, Pattern_arr; model = :Heisenberg_square, Space_type = ℂ, loc_terms = true, nn_terms = true, nnn_terms = true, u_paras = false)
    ...
    if u_paras != false
        ham_term_array = apply_loc_unitaries(ham_term_array, u_paras; space_type = Space_type)
    end
    ...
    for i in minimum(Pattern_arr):maximum(Pattern_arr)
        ...
        if haskey(ham_term_array[i], :loc)
            ...
            #display("site $i has a local term and we calculate its energy and add it s th e = $(energy_density_local)")
    ...
    for i in minimum(Pattern_arr):maximum(Pattern_arr)
        ...
        if haskey(ham_term_array[i], :hor)
            ...
            #display("site $i has a hor term and we calculate its energy and add it s th e = $(energy_density_nn)")

        end
        #here we calculate the vertical expectation value
        if haskey(ham_term_array[i], :vert)
            ...
            #display("site $i has a vert term and we calculate its energy and add it s th e = $(energy_density_nn)")
    ...
    for i in minimum(Pattern_arr):maximum(Pattern_arr)
        ...
        if haskey(ham_term_array[i], :diag)
            ...
            #display("site $i has a diag term and we calculate its energy and add it s th e = $(energy_density_nnn)")

        end
    
    end
    
    energy_density_box = 0
    ...
    for i in minimum(Pattern_arr):maximum(Pattern_arr)
        ...
        if haskey(ham_term_array[i], :box)
            ...
            if haskey(ham_term_array[1], :box)

                @tensor order = (q1, w3, x3d, p1d, x3u, p1u,     v1, w1, x1d, u1d, x1u, u1u,    w2, x2d, x2u,     v3, z1, u3d, y1d, u3u, y1u,     z3, q3, p3d, y3d, p3u, y3u,     z2, y2d, y2u,     v2, u2u, u2d, p2u, p2d, q2,   r1,r2,d1,d2, l1, l2, dr1, dr2) begin 
            
                    exp_val_box[] := env_loc.ul[v1,w1] * env_loc.u[w1,u1d,u1u,w2] * env_loc_r.u[w2,p1d,p1u,w3] * env_loc_r.ur[w3,q1] * 
            ...
            energy_density_box += TensorKit.scalar(exp_val_box) / TensorKit.scalar(norm_dr)

        end
    
    end

    energy_density_tot_per_site = (energy_density_local + energy_density_nn + energy_density_nnn + energy_density_box) / length(minimum(Pattern_arr):maximum(Pattern_arr))

    #display(energy_density_tot_per_site)
